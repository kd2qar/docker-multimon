...
static const t_init_procs init_procs[] = {
	gen_init_dtmf, gen_init_sine, gen_init_zvei, gen_init_ccir, gen_init_hdlc
};

static const t_gen_procs gen_procs[] = {
	gen_dtmf, gen_sine, gen_zvei, gen_ccir, gen_hdlc
};
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "t:a:d:s:z:c:p:")) != EOF) {
		...
		case 'c':
			...
			params[num_gen-1].type = gentype_ccir;
			params[num_gen-1].ampl = 16384;
			params[num_gen-1].p.ccir.duration = MS(100);
			params[num_gen-1].p.ccir.pause = MS(10);
